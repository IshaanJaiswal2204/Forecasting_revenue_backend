...
@Repository
public class BaselineRevenueJdbcDAO {

    private final JdbcTemplate jdbcTemplate;

    public BaselineRevenueJdbcDAO(JdbcTemplate jdbcTemplate) {
        this.jdbcTemplate = jdbcTemplate;
    }

    public String getTableName(int year, int month) {
        return "baseline_revenue_" + year + "_" + String.format("%02d", month);
    }
    ...
    public void batchInsertOrUpdate(String tableName, List<BaselineResponseDTO> dtos, int year, int month) {
        for (BaselineResponseDTO dto : dtos) {
            ...
            if (count != null && count > 0) {
                String updateSql = "UPDATE " + tableName + " SET " +
                        "service_line = ?, project_id = ?, project_name = ?, city = ?, hours = ?, " +
                ...
                String insertSql = "INSERT INTO " + tableName + " (" +
                        "associate_id, associate_name, service_line, project_id, project_name, city, hours, " +
